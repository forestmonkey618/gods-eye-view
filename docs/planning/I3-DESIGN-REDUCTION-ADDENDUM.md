...
**Status:** I3a IMPLEMENTED — minimal provenance primitive + aircraft position provenance. Previous research phase closed.
**Date:** 2026-09-18 I3a slice. Evidence from actual checkout at `arena/01a0b2f3-gods-eye-view` traced server/providers/aircraft/opensky.js → API/route → X-Flight-Source handling → client src/sources/live/standalone.js → FlightRecords.receive() → getCurrentEntities().
**Purpose:** Reduce speculative 7-class model to smallest correct substrate before owner approval, then implement minimal vertical slice truthfully.
