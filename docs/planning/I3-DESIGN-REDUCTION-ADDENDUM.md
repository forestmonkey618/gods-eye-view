...
**Status (2026-09-24, I3a finalization):** the provenance authority contract is frozen against the production stores as-is (no primitive change). Added: `docs/PROVENANCE.md` (current-state reference — field semantics, per-feed facts, known gaps; CURRENT-STATE.md and CODE-BOUNDARIES.md entries), `scripts/check-provenance-authority.mjs` wired into `check:boundaries` (the post-I3b freeze planned in §28 — authority purity, locked vocabulary, single construction path, I2/freshness separation, Cesium/DOM-free record stores), and focused negative tests (malformed optional metadata rejected, missing source time never filled by wall clock or receipt, observation input not mutated, rendering visibility no-effect, determinism). A vessel record-level provenance slice (I3d) is present in `src/layers/vessels/records.js` with tests and is documented in PROVENANCE.md; it has no dedicated implementation report yet.
